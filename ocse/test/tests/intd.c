...
#define DEVICE "/dev/ocxl/tlx0.0000:00:00.1.0"
...
int main(int argc, char *argv[])
{
    ...
    rc = ocxl_afu_open_from_dev(DEVICE, &mafu_h);
    if(!mafu_h) {
...
//    ocxl_mmio_write64( mafu_h, ProcessInterruptData_REGISTER, 0x00000000);
    ...
    config_param.mem_size = 8;
    config_param.command = AFU_CMD_INTRP_REQ_D;
    config_param.mem_base_address = (uint64_t)irq_h;
    config_param.mem_dest_address = 0x11223344aabbccdd;
